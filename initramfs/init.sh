...
check_disable_lagfix() {
	...
	if ext4_check; then
		...
		# unmount /dbdata but leave /data mounted
		umount /dbdata
...
# check if disable-lagfix is present and disable lagfix if so
check_disable_lagfix
...
# exit after handling the log files
umount /sdcard
letsgo
